...
| department | Department | Link | N | Department A | must exist if set | stock optional |
| designation | Designation | Link | N | Designation A | must exist if set | stock optional |
| reports_to | Reports to | Link | N | HR-EMP-002 | must exist if set | self-link to another Employee |
| branch | Branch | Link | N | Branch A | must exist if set | stock optional; unused at single-site (gotcha #12) |
| holiday_list | Holiday List | Link | N | Holiday List A | must exist if set | stock optional but functionally required |
...
| employment_type | Employment Type | Link | N | Full-time | must exist if set | custom field; link to Employment Type master |
| grade | Grade | Link | N | Grade A | must exist if set | custom field; unused at single-site (gotcha #13) |
| default_shift | Default Shift | Link | Y* | T1 | must exist | custom field; required if auto-attendance enabled (gotcha #6) |
| health_insurance_provider | Health Insurance Provider | Link | N |  | must exist if set | custom field |
| health_insurance_no | Health Insurance No | Data | N |  | <= 50 chars | custom field |
| leave_approver | Leave Approver | Link | N | Administrator | must exist if set | custom field; link to User |
| expense_approver | Expense Approver | Link | N | Administrator | must exist if set | custom field; link to User |
| shift_request_approver | Shift Request Approver | Link | N | Administrator | must exist if set | custom field; link to User |
...
| salary_currency | Salary Currency | Link | N |  | must exist if set | stock optional |
