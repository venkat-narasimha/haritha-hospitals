...
if current >= total:
    ...
    for batch_end in range(start + BATCH_SIZE, total + BATCH_SIZE, BATCH_SIZE):
        ...
        for idx, row in enumerate(batch):
            ...
            # Name format: HR-ATT-YYYYMMDD-NNNNN (matching Frappe default autoname pattern)
        ...
        if not values_list:
            ...
            continue
        sql = "INSERT INTO `{}` (name, employee, attendance_date, status, shift, leave_type, leave_application, late_entry_by, early_out_by, is_wfh, company, owner, modified_by, creation, modified) VALUES {}".format(
